...
class Seeder {
  static async seed() {
    ...
    const faceOff = await Movie.query().insert({ title: "Face Off", releaseYear: 1997, genreId: action.id })
    const pulpFiction = await Movie.query().insert({ title: "Pulp Fiction", releaseYear: 1994, genreId: drama.id })
    const avengers = await Movie.query().insert({ title: "The Avengers", releaseYear: 2012, genreId: action.id })
    const manOnTheMoon = await Movie.query().insert({ title: "Man on the Moon", releaseYear: 1999, genreId: drama.id })
    const mask = await Movie.query().insert({ title: "The Mask", releaseYear: 1994, genreId: comedy.id })
    ...
    await faceOff.$relatedQuery("reviews").insert({ title: "SO weird", content: "gotta love Nick Cage though", rating: 2 })
    await manOnTheMoon.$relatedQuery("reviews").insert({ title: "Wow!", content: "Jim Carrey at his best", rating: 5 })
    await manOnTheMoon.$relatedQuery("reviews").insert({ title: "Moving Movie", content: "See what I did there?", rating: 4 })
    await mask.$relatedQuery("reviews").insert({ title: "SMOKING", rating: 3 })
